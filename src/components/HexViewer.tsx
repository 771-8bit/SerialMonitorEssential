    ...
    return (
        <div className="hex-viewer">
            ...
            <div className="hex-footer">
                <span>Total: {totalBytes.toLocaleString()} bytes ({totalRows.toLocaleString()} rows)</span>
                <span style={{ marginLeft: 10, color: '#888' }}>Row: {currentStartRow.toLocaleString()}</span>
                {scale < 1 && <span style={{ marginLeft: 10, color: '#f80' }}>(scale: {(scale * 100).toFixed(1)}%)</span>}
            </div>
        </div>
    );
}
