        ...
        <div className="settings-panel">
                    ...
                    {baudDropdownOpen && !baudRateEditing && !isConnected && (
                        <>
                            ...
                            <ul style={{
                                ...
                                {[9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600, 1000000, 2000000, 3000000, 12000000].map(rate => (
                                    <li
                                        key={rate}
                                        onClick={() => { handleChange("baud_rate", rate); setBaudDropdownOpen(false); }}
                                        style={{
                                            padding: '4px 8px',
                                            cursor: 'pointer',
                                            fontSize: '13px',
                                ...
                                <li
                                    onClick={() => { setBaudRateEditing(true); setBaudDropdownOpen(false); }}
                                    style={{
                                        padding: '4px 8px',
                                        cursor: 'pointer',
                                        fontSize: '13px',
            ...
            <div className="settings-advanced">
                ...
                <div className="advanced-group">
                    ...
                    <select
                        value={config.parity}
                        onChange={(e) => handleChange("parity", e.target.value as any)}
                        disabled={isConnected}
                    >
                        {["None", "Odd", "Even"].map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                </div>
                ...
                <div className="advanced-group">
                    ...
                    <select
                        ...
                        onChange={(e) => handleChange("flow_control", e.target.value as any)}
                        disabled={isConnected}
                    >
                        {["None", "Software", "Hardware"].map(f => <option key={f} value={f}>{f}</option>)}
                    </select>
                </div>
            </div>
        </div>
