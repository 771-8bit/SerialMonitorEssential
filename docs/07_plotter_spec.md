...
### 技術選定
*   **ラインチャート:** [uPlot](https://github.com/leeoniya/uPlot) - 圧倒的なパフォーマンスのため採用（インストール済み）。
*   **ステートタイムライン:** uPlotのカスタムプラグインでの実装を検討中（実装時に判断）。
...
*   **作業内容:** uPlotカスタムプラグインまたはカスタムコンポーネントで状態変化を可視化（実装時に判断）。
*   **中間確認:**
    *   [ ] 状態がカラーバーで表示される
    *   [ ] 複数チャンネルが行として表示される
    *   [ ] 状態名がバー内に表示される
