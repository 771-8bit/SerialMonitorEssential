...
| 種別 | 件数 | 内訳 |
|------|------|------|
| Rust 例示テスト `#[test]` | **220** | aggregator 37 / data_store 36 / parser 31 / **mcp_stdio 34** / **bridge 30** / thread 22 / logger 8 / worker 8 / chunk 6 / serial::mod 6 / state_transition 2 |
| Rust プロパティテスト `proptest` | **11** | aggregator 7 / parser 3 / data_store 1 |
| Rust 合計（`cargo test --lib` 実行結果） | **231 passed** | |
...
| **GAP-31** | **ほぼ解消（2026-09-03）**: ~~AI Bridge の E2E がハーネスに入っておらず手動実施の記録のみ~~ → (a) 実往復 E2E をスクリプト化し `test_tools/e2e/` に収載（`run_bridge_e2e.ps1` = 起動→UIA 設定→`pong_bot.py`→`mcp_bridge_live.py` の一括実行。status/send→wait_for/read_tail/ブロック中 ping 即応答/cancelled 中断を検証）、(b) GUI 不要の `mcp_stdio_smoke.py` を **CI（Tier 1、両 OS）に組み込み**（実バイナリ `--mcp` を実パイプで検証）、(c) 実 `BridgeServer` への自動統合テスト（`test_tcp_client_against_real_bridge` 等 3 件）。**残**: Node 版 `mcp/smoke.mjs` は偽ブリッジに対する結合のまま（開発用リファレンスとして許容、CI には組み込み済み）。トークン設定 UI 未提供は TBD-R7 として別管理 | SYS-F-1101〜1107 | 低 |
...
| 順位 | GAP | 理由 |
|------|-----|------|
| — | GAP-02, GAP-03, GAP-04, GAP-05, GAP-07, GAP-08, GAP-09, GAP-11, GAP-28, GAP-30, GAP-31 | **解消済**（GAP-02/03/05/11/28/30 は 2026-09-03 前半、GAP-04/07/08/09 は AI Bridge 回、GAP-31 は E2E スクリプト収載 + stdio smoke の CI 化でほぼ解消・残件は表の記載どおり） |
...
| 3 | GAP-12 の残り | パーサ・集約器は整備済。残るのはパーサ以外の panic 耐性（P-2 / P-6） |
| 4 | GAP-22 の残り | Y 軸ヒステリシスのタイミング検証（`calculateYRange` 分は 2026-09-03 に解消） |
| 5 | GAP-29 | 上記を整備した後、スイート全体の有効性を測る基線として |
