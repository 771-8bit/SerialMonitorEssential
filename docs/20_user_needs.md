...
### US-07: AI エージェントと一緒にデバッグする

アプリを開いたまま、手元の AI エージェント（Claude Code 等）に
「`AT+VER` を送ってバージョンを読んで」「今のログから異常行を探して」と頼む。
利用者はその間 GUI で同じストリームを眺めており、**AI が何を送ったかを画面で確認できる**。
COM ポートは排他なので、AI に別プロセスでポートを開かせることはできない。
...
| **UN-19** | 利用者は、デバイスが**物理的に外れたこと・新しく挿さったこと**を検知して、UI がその旨を示すことを必要とする | USB は抜ける。抜けたのに「接続中」と表示され続けると、来ないデータを待ち続けることになる。逆に、挿したのにポート一覧に出てこなければ再起動するしかない | US-02 | 重要 |

> **UN-19 の実現方針（決定 2026-09-03）**: 「read エラーによる切断検知」に加えて、
> **ポート一覧の能動ポーリング（2 秒間隔）を実装する**（GAP-07 の判断保留を解除）。
> 切断の権威は read エラー経路に残し、ポーリングは**列挙の更新だけ**を担う。
> 接続中に選択中のポートが黙って別のポートへ移らないことを条件とする。
> → [21 SYS-F-107](21_system_requirements.md#a1-シリアル接続-1xx)
...
| ID | ニーズ | 根拠 | 由来 | 優先度 |
|----|--------|------|------|--------|
| **UN-24** | 利用者は、**人間が GUI で監視したまま、手元の AI エージェントに同じシリアルセッションを読み書きさせられる**ことを必要とする。AI による送信は、常に人間の画面から見えていること | COM ポートは OS レベルで排他であり（AS-3）、AI に別プロセスでポートを開かせることはできない。**アプリ自身がマルチプレクサになる以外に実現手段がない。**「AI に打たせて人が横で確認する」が成立するには、AI の送信が黙って行われないことが前提になる | US-07 | 重要 |
| **UN-25** | 利用者は、**インストーラで入れただけの環境で**（リポジトリのクローンや Node.js 等の追加ランタイム無しに）AI 連携をセットアップできることを必要とする。その手順は**アプリ自身から参照できる**こと | GUI の他の機能はボタン操作で自明だが、AI 連携だけは外部クライアントへの登録という不可視の手順を要する。README はリポジトリに居る人にしか届かず、インストール先の exe パスはユーザーごとに異なるため、**アプリが実際のパス入りの手順を提示するしかない**（オーナー指摘 2026-09-03） | US-07 | 重要 |

> **実現方針（決定 2026-09-03）**: **ローカルブリッジ + MCP の 2 層**で実装する。
> アプリ内に localhost 限定のブリッジを置き、外付けの MCP アダプタがそれを AI 向けのツールへ変換する。
> 単一層（アプリ内に MCP サーバを内蔵する）は採らない。→ [22 ADR-12](22_architecture_description.md#adr-12-ai-ポートをアプリ内マルチプレクサ--外付け-mcp-アダプタの-2-層にする)
>
> **追補（2026-09-03, UN-25 対応）**: MCP アダプタの**配布形態**はアプリ本体への内蔵
> （`--mcp` フラグで同一 exe が別プロセスとして MCP stdio サーバになる）。2 層構造
> （ブリッジ ↔ MCP アダプタが別プロセス）は維持したまま、Node.js 依存を排した。
> → [22 ADR-13](22_architecture_description.md)
...
| AS-3 | COM ポートは OS レベルで排他オープンである。アプリが開いている間、同じポートを他プロセスが開くことはできない | 前提 |
