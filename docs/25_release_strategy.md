...
| GAP | 内容 | 関連要求 | 1.0 での扱い（暫定。タグ前に確定させる） |
|-----|------|----------|------------------------------------------|
| GAP-01 | 検索・フィルタ未実装（UI のみ存在） | SYS-F-309 | 実装。UI が存在して動かない状態は誤解を招く（動かないなら UI ごと外す） |
| GAP-04 | min/max バンドが Y オートレンジに含まれない | SYS-F-525 | 実装。UN-03（スパイクを消さない）に直結する |
| GAP-06 | プロッタのドッキング / フロート切替 | SYS-F-905 | スコープ外化の候補。別ウィンドウで用は足りている |
| GAP-07 | ホットプラグの能動検知 | SYS-F-107, DEBT-2 | 判断保留。read エラー検知で実害が出るか実機で確認してから決める |
| GAP-08 | 切断検知後もポートハンドルを保持（UI と不一致） | SYS-F-107 | 実装。UN-19 の期待と食い違う |
| GAP-09 | `log-error` イベントの発火経路が未実装 | SYS-F-205 | 実装。ディスクフルが黙って失敗するのは UN-10 に反する |
