# よく使うプロンプト集
## Continuous
```
npm run lint
npm run lint:css-unused
npm run type-check
npm run format
cd src-tauri
cargo fmt
cargo clippy
cargo test
cargo check
```
このエラーを修正してください

npm run test:coverage
カバレッジレポートを見て，カバレッジが100%に近くテストの追加が簡単そうなところからテストを追加してください．テストを追加する中で，問題が起きた場合はテストが間違っているのかプロダクトコードが違うのか慎重に判断してください．意図的なdeadcodeであればそれを明記してください．

cargo llvm-cov --lib
カバレッジレポートを見て，カバレッジが100%に近くテストの追加が簡単そうなところからテストを追加してください．テストを追加する中で，問題が起きた場合はテストが間違っているのかプロダクトコードが違うのか慎重に判断してください．意図的なdeadcodeであればそれを明記してください．

eslint-disableの使用は適切ですか？本当にeslint-disableをするのが避けられないのか，本質的に不適切なコードなのか全て調べてください．

clippyの警告を発生させないようにallowなどをしている箇所がありますが，これは適切ですか？本当に避けられないのか，本質的に不適切なコードなのか，今後実装が必要なのか全て調べてください．

現状ほぼ全てのテストに通っていますが，プロダクトのコードもテストコードも両方間違っている可能性も存在します．テストコードをレビューして，問題がなさそうか確認してください．問題がなくても，単体テストの設計が不適切であればリファクタリング方針を示してください．

ドキュメントを整理してください．
docsフォルダを読み，実際のコードと見比べ，記述を追記編集削除してください．
ドキュメントに明示されていないコードの説明を追記してください．
また，コード内のコメントが間違っていたり不正確だったりしていた場合は訂正してください．
実装計画について，対応が完了しているドキュメントは削除してください．今後実装が必要なものの計画は残してください．
実装計画ではなくリポジトリの説明をしているドキュメントは現在のコードに整合させてください．
複数のドキュメントに同じ内容が記述されている場合は，一つにまとめてください．

```
npx license-checker --production --markdown | grep -v "\[tauri-appserial-monitor-essential@" > docs/THIRD_PARTY_NPM.md
cd src-tauri
cargo about generate about.hbs > ../docs/THIRD_PARTY_RUST.md
```

## 要件定義時
この内容のAIコーディングエージェントへの実装指示書を書いてください．
AIコーディングエージェントは現在のリポジトリのコードを把握しています．また，追加機能もコードではなく日本語で説明すべきです．コードよりも背景や目的，全体の流れなど，ここで議論した内容を伝えたいです．それをふまえてプロンプトを作ってください．

詳細をAIエージェントに伝える前に，仕様はもう少し決めなくていいですか？これで実装に進めますか？

～の内容を精査し，現状のコードを理解したうえで，実装書としての内容が適切か判断してください．

## 機能追加時
～の通り，実装を行ってください．
～の通り，続けて実装を行ってください．

どのような計画か日本語で説明してください．

動作確認としては何を検証すべきですか？
私が手を動かすので日本語で確認内容の指示を出してください．

適切にテストも追加してください．

この仕様をvitestでテストすることは可能ですか？

テストコード側ではなくプロダクトコード側をいじっていますが本当に大丈夫ですか？関連したところの実際の動作テストをしたいため，どのような操作をすればいいか指示してください．

正常に動作しています．ドキュメントを更新したうえで作業を終了してください．
実装にあたり，仕様を変更している場合はそれも反映させてください．

lint，type-check，testそれぞれパスしています．実装を続けてください．

以下について，それぞれコメントを鵜呑みにせずコードをしっかり読んで回答してください．
1. の設計はバッファ管理，受信やロギング，UIを含めたスレッド設計に問題点は全くないか
2. Phase4をこれから実装するのにあたり現在のコードが適切なコードになっているか，余計なコードや依存関係が混入していたり，Phase3までの実装が計画と合わない実態になっていないか，

## リファクタ時
設計や実装のレビューをしてください．
docsフォルダを参考にしつつ，リファクタリングの方針をdocsフォルダにmarkdownで書いてください．
逆に全て問題がない場合にはどのような視点で何をチェックして問題なかったのか明示し，漏れがないことを確認してください．

～別のマークダウンファイルをdocsフォルダに作ってください．
現在の実装と問題点，リファクタリングの方針，具体的な実装手順をまとめてください．

～を見て，～のリファクタリングを行ってください．
方針について質問があれば聞いてください．

を見て，現在の進捗状況に対し，実際のコードがどうなっているか確認してください．
実装として不適切な点がないか，ドキュメントにある設計や進捗に不整合がないか確認してください．
リファクタリングの必要があればその方針を示し，テストやCIが必要であれば追加してください．

の内容を精査し，現状のコードを理解したうえで，リファクタリングが適切に終了しているか判断してください．
リファクタリングが終了していれば，ドキュメントの内容のうち今後も参照する必要がある情報を既存のドキュメントに書き写してください．
