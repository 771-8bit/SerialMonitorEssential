...
### Phase 1: 疎通確認とシリアルポート基盤構築 (Tracer Bullet)

*   **目標:** `serialport` クレートを使用してCOMポートを開き、12Mbpsの設定投入と、基本的なReadができることを確認する。UIへの表示はコンソールまたは最低限のTauri画面で行う。

> [!NOTE]
> 当初は `windows` クレートによるWin32 API直接制御で実装しましたが、クロスプラットフォーム対応のため `serialport` クレートに移行しました。
> **重要:** Arduino等のUSB-Serialデバイスはデータ送信にDTR/RTS信号が必要です。ポートオープン後に `write_data_terminal_ready(true)` と `write_request_to_send(true)` を呼び出すこと。

#### 1-1. Rustプロジェクトセットアップ
*   **作業内容:** `serialport` クレートの導入、Tauriプロジェクトの初期化。
*   **中間確認:**
    *   [x] `cargo build` が成功する
...
*   **中間確認:**
    *   [x] Rust側で `serialport::available_ports()` を用いてポート一覧を取得できる
    *   [x] Tauri Command `list_ports` でフロントエンドからポート名一覧を取得できる

#### 1-3. SerialPort構造体（Create/Close）
*   **作業内容:** `serialport::new().open()` でポートを開き、Drop時に自動クローズするラッパー実装。
*   **中間確認:**
    *   [x] 存在するCOMポートを開ける
    *   [x] 存在しないポートを開こうとすると適切なエラーが返る
    *   [x] Drop後に再度開けることを確認（リソースリーク確認）

#### 1-4. SerialPort構造体（設定）
*   **作業内容:** `serialport::new()` のビルダーパターンで設定投入。
*   **中間確認:**
...
#### 1-5. Basic Reader
*   **作業内容:** `port.read()` を用いた単純な受信ループの実装。
*   **中間確認:**
...
#### 4-1. ポート一覧のオンデマンド更新
*   **作業内容:** ユーザーがポート選択ドロップダウンを開いた際に `serialport::available_ports()` を呼び出し、最新のポート一覧を取得する。
*   **設計方針:**
    * クロスプラットフォーム対応のため、OS固有のデバイス検知APIは使用しない
    * ドロップダウンを開くたびに `list_ports` コマンドを呼び出す
    * オプション: バックグラウンドで定期的にポーリング（1秒間隔）し、UI側でキャッシュを更新
*   **中間確認:**
    *   [x] ドロップダウンを開くと最新のポート一覧が表示される
    *   [x] デバイス挿入後にドロップダウンを開くと新しいポートが表示される
    *   [x] デバイス抜去後にドロップダウンを開くと削除されている

#### 4-2. 安全な切断処理
*   **作業内容:** デバイス切断時に安全にポートを閉じ、UIに通知する。
*   **中間確認:**
    *   [x] 受信中に切断してもアプリがクラッシュしない
    *   [x] 切断イベントがフロントエンドに通知される
    *   [x] 受信スレッドが正しく終了する
...
*   **中間確認:**
    *   [x] ファイル保存ダイアログが表示される
    *   [x] 指定パスにファイルが保存される
    *   [x] 保存されたファイルの内容が正しい（バイナリエディタで確認）
...
*   [x] **切断耐性:** 受信中にUSBケーブルを物理的に引き抜き、アプリがクラッシュせず、エラー表示に切り替わること。
*   [x] **再接続:** 再度ケーブルを挿した際、手動で再接続できること。
*   [x] **ログ検証:** エクスポートしたログファイルをバイナリエディタで開き、破損がないか確認。

---

### Phase 5: 送信機能の実装 (Sending Capability) ✓ 完了

*   **目標:** テキストボックスからのデータ送信機能を実装する。巨大なデータの送信には対応せず、入力されたデータをそのまま送信する。

#### 5-1. 送信ロジック実装 ✓
*   **作業内容:** `serialport` クレートの `write` メソッドを用いたデータ送信実装。
*   **中間確認:**
    *   [x] テキストデータを送信できる
    *   [x] `port.write` がエラーなく完了する

#### 5-2. 送信UI ✓
*   **作業内容:** 送信エリアのUI実装。
    *   機能: テキスト入力フォーム、送信ボタン、Enter送信オプション、バイナリ送信オプション、改行コード選択。
    *   送信履歴機能（Up Arrow）。**※重複コマンドは履歴に追加しない仕様とした。**
*   **中間確認:**
    *   [x] テキスト入力欄から送信ボタンでデータが送れる
    *   [x] Enterキーで送信できる（オプション有効時）
    *   [x] Hexモード等の送信オプションが機能する
    *   [x] 送信履歴が呼び出せる

#### Phase 5 完了条件 (Verification) ✓
*   [x] **ループバックテスト:** TX-RXを短絡し、送信したデータが即座に受信ビューアに表示されること。
*   [x] **履歴重複排除:** 連続して同じデータを送信した場合、履歴に重複して記録されないこと。

---

### Phase 6: フロントエンド刷新とUI調整 (UI Overhaul)

*   **目標:** 全体的なUIを見直し、提供されたデザインモックアップ（スクリーンショット）に合わせてリファインする。

#### 6-1. 全体レイアウトの調整
*   **作業内容:** 画面構成を「Settings」「Send」「Receive」の3セクションに整理し、洗練された見た目にする。
    *   **Settings:** Port, Baud, Connect, Auto Connect, Advanced (Data bits, Parity, DTR, RTS, Stop bits, Flow control)
    *   **Send:** Input area, Send button, Options (Send with Enter, Send Binary, Line ending)
    *   **Receive:** Log viewer, Controls (Line Wrap, Binary view, Show CRLF/NUL, Auto Scroll, Timestamp, Save, Copy, Clear)
*   **中間確認:**
    *   [ ] モックアップ通りのレイアウトが組まれている
    *   [ ] 各コントロールが適切な位置に配置されている

#### 6-2. 機能の再配置と接続
*   **作業内容:** 既存機能を新しいUIコンポーネントに接続し直す。
*   **中間確認:**
    *   [ ] 新しい「Connect」ボタンで接続・切断ができる
    *   [ ] Advanced設定が正しく反映される

#### Phase 6 完了条件 (Verification)
*   [ ] **デザイン整合性:** 提供されたUI画像と実装された画面が視覚的に一致していること。
*   [ ] **操作性:** 全てのUI要素が期待通りに反応し、既存機能が壊れていないこと。

---

### Phase 7: リアルタイムグラフ描画 (Real-time Plotting)

*   **目標:** 受信データから数値をパースし、グラフとして可視化する。

#### 7-1. 数値パースロジック
*   **作業内容:** 受信ストリームから数値（CSV, JSON, バイナリ等）を抽出するパーサー実装。
*   **中間確認:**
    *   [ ] 混合データから数値を正しく抽出できる
    *   [ ] パースエラー時にクラッシュしない

#### 7-2. グラフ描画 (`uPlot` 導入)
*   **作業内容:** `uPlot` (または適切な軽量ライブラリ) を導入し、リアルタイム描画を行う。
*   **中間確認:**
    *   [ ] 抽出したデータがグラフにプロットされる
    *   [ ] リアルタイムで更新される

#### 7-3. パフォーマンスチューニング
*   **作業内容:** 高速受信時 (12Mbps) の描画負荷対策（間引き、Batch update）。
*   **中間確認:**
    *   [ ] 高負荷時でもUIがフリーズしない
    *   [ ] 描画がデータの流れに追従する

#### Phase 7 完了条件 (Verification)
*   [ ] **グラフ追従性:** 高速データ更新時にグラフ描画が遅延なく追従するか。
